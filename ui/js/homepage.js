            ...
            .then(data => {
                ...
                if (data && data.success) {
                    ...
                    if (data.requiresVerification) {
                        // Store user ID, email, and role for verification
                    ...
                    // keep user object (with role/id/department) for other pages
                    try {
                        const userObj = data.user || { email };
                        // Ensure we capture department from response
                        if (data.user && !userObj.department) {
                            userObj.department = data.department || '';
                        }
                        console.log('[LOGIN] Storing user object:', userObj);
                    ...
                    fetch('https://ipapi.co/json/')
                        ...
                        .finally(()=> {
                            ...
                            if (!target) {
                                ...
                                if (roleLC === 'staff') target = '/ui/staff.html';
                                else if (roleLC === 'admin') target = '/ui/admin.html';
                                else target = '/ui/dashboard.html'; // super_admin
                            }
                            console.log('navigating to', target, 'with role:', role);
                            // Use direct navigation instead of window.open for reliable redirect
...
function processTicketSignIn(email, button = null) {
    ...
    // oneâ€‘time code.  The server has been adjusted to issue a code even for already
    // verified addresses.
    ...
    sendVerificationCode(email, 'ticket', button);
}
...
if(gmailLoginForm) {
    gmailLoginForm.addEventListener('submit', function (e) {
        ...
        // Get the continue button and set loading
        const continueBtn = e.target.querySelector('button[type="submit"]');
        if (continueBtn) {
            continueBtn.disabled = true;
            continueBtn.innerHTML = '<span class="signin-spinner" aria-hidden="true"></span>Continuing...';
        }
        
        // Process the sign-in
        processTicketSignIn(gmailEmail, continueBtn);
    });
}
...
if(ticketForm) {
    ...
    let submitButtonPrevText = '';
    if (submitButton) {
        submitButtonPrevText = submitButton.textContent;
        submitButton.disabled = true;
        submitButton.innerHTML = '<span class="signin-spinner"></span>Submitting...';
    }
    ...
    .then(data => {
        if (data && data.success) {
            const serverTicketId = (data.data && (data.data.ticket_id || data.data.ticketId)) ? (data.data.ticket_id || data.data.ticketId) : ('TKT-' + Date.now());
            ...
            showTicketSuccessModal(serverTicketId, currentTicketEmail);
    ...
    .finally(() => {
        if (submitButton) {
            ...
            submitButton.innerHTML = submitButtonPrevText || 'Submit Ticket';
        } else {
...
function sendVerificationCode(email, context = 'login', button = null) {
    if (button) {
        button.disabled = true;
        button.innerHTML = '<span class="signin-spinner" aria-hidden="true"></span>Sending...';
        ...
        // Verify code with backend
            ...
            if (data && data.success) {
                ...
                if (context === 'login') {
                    ...
                    // Get role from either response or stored value
                    let role = (data.user && data.user.role) || sessionStorage.getItem('userRoleForVerification') || 'super_admin';
                    sessionStorage.removeItem('userRoleForVerification');
                    
                    // Store user object for other pages
                    try {
                        const userObj = data.user || { email, role };
                        localStorage.setItem('user', JSON.stringify(userObj));
                    } catch (e) { console.warn('Could not store user object', e); }
                    
                    // Show success message
                    loadingDiv.innerHTML = '<h2 style="color: #003f8f; text-align: center; margin: 0;">âœ“ Email Verified!</h2><p style="text-align: center; margin-top: 20px;">Redirecting to dashboard...</p>';
                    
                    // Redirect after a short delay with role-based routing
                    setTimeout(() => {
                        const roleLC = role.toLowerCase();
                        let target = '/ui/dashboard.html'; // default to super_admin
                        if (roleLC === 'staff') target = '/ui/staff.html';
                        else if (roleLC === 'admin') target = '/ui/admin.html';
                        
                        console.log('[VERIFY] Redirecting with role:', role, 'to:', target);
                        window.location.href = target;
                    }, 1500);
                } else if (context === 'ticket') {
                    // TICKET CONTEXT: show ticket form (don't log in)
                    loadingDiv.innerHTML = '<h2 style="color: #003f8f; text-align: center; margin: 0;">âœ“ Email Verified!</h2>';
...
// Track Ticket Modal functionality
document.addEventListener('DOMContentLoaded', function() {
    ...
    const resultAcceptedBy = document.getElementById('resultAcceptedBy');
    const resultCreatedAt = document.getElementById('resultCreatedAt');

    if (!trackModal || !trackForm) {
        console.error('Track ticket elements not found');
        return;
    }
    ...
    trackForm.addEventListener('submit', function (e) {
        e.preventDefault();
        const ticketId = trackTicketId.value.trim();
        if (!ticketId) {
            trackError.textContent = 'Please enter a ticket ID.';
            trackError.classList.remove('hidden');
            trackTicketId.focus();
            return;
        }

        fetch(`http://localhost:8000/api/track-ticket?ticket_id=${encodeURIComponent(ticketId)}`)
        .then(resp => resp.json())
        .then(data => {
            if (data.success) {
                const ticket = data.ticket || {};
                resultTicketId.textContent = ticket.ticket_number || ticket.ticket_id || ticket.ticketId || 'N/A';
                resultCustomerName.textContent = ticket.customer_name || ticket.customerName || 'N/A';
                resultEmail.textContent = ticket.email || 'N/A';
                resultBranch.textContent = ticket.branch || 'N/A';
                resultDepartment.textContent = ticket.department || 'N/A';
                resultCategory.textContent = ticket.category || 'N/A';
                resultDescription.textContent = ticket.description || 'N/A';
                resultStatus.textContent = ticket.status || 'N/A';
                resultAcceptedBy.textContent = ticket.accepted_by || ticket.acceptedBy || 'Not assigned';
                resultCreatedAt.textContent = new Date(ticket.created_at || ticket.createdAt).toLocaleString();
                trackResult.classList.remove('hidden');
                trackError.classList.add('hidden');
                openTrack();
            } else {
                trackError.textContent = data.message || 'Ticket not found.';
                trackError.classList.remove('hidden');
                trackResult.classList.add('hidden');
            }
        })
        .catch(err => {
            ...
            trackError.textContent = 'Network error. Please try again later.';
            trackError.classList.remove('hidden');
            trackResult.classList.add('hidden');
        });
    });
    ...
    // Close button
    const closeBtn = trackModal.querySelector('.modal-close');
    if (closeBtn) {
        closeBtn.addEventListener('click', closeTrack);
    }
});

// Show success modal after ticket creation
function showTicketSuccessModal(ticketId, email) {
    ...
    if (!successModal || !successTicketId || !successEmail || !successOkBtn) {
        console.warn('Success modal elements missing - fallback to alert');
        alert(`Ticket created successfully!\nTicket ID: ${ticketId}`);
        return;
    }

    successTicketId.textContent = ticketId;
    successEmail.textContent = email || 'N/A';
    ...
    successModal.querySelectorAll('.modal-close').forEach(btn => {
